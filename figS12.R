...
## load data
data <- read.table("/Users/au612643/Desktop/par/rScripts/github/figS12.txt", header=T, sep = "\t")
...
fS12e <- ggplot(subset(data, pop == "PAB"), aes(x=log2(cMmb), y=log2(freq),colour = mType,))  + geom_point(size=0.005,alpha=0.2) + 
  geom_smooth(method="lm", lty =2, lwd=0.7) +theme_bw() + theme(legend.title = element_blank())+
  xlab(expression(italic(log[2])(cM/Mb))) + ylab(expression(italic(log[2])(italic(f[GC])))) +
  scale_color_manual(values = c("#377eb8", "#e41a1c", "#4daf4a"))
...
fS12f <- ggplot(subset(data, pop == "PAB"), aes(x=log2(cMmb), y=log2(count),colour = mType,))  + geom_point(size=0.005,alpha=0.2) + 
  geom_smooth(method="lm", lty =2, lwd=0.7) +theme_bw() + theme(legend.position = "none")+
  xlab(expression(italic(log[2])(cM/Mb))) + ylab(expression(italic(log[2])(Count))) +
  scale_color_manual(values = c("#377eb8", "#e41a1c", "#4daf4a"))
...
tt <- plot_grid(fS12a, fS12b, legfS12a,
                ...
                fS12e, fS12f, legfS12e, ncol = 3, align="bt", rel_widths = c(2,2,0.8), labels = c("A", "B", "", "C", "D", "", "E", "F", ""))
ggsave("/Users/au612643/Desktop/par/rScripts/github/figS12.pdf", tt, width=18.5, height=18, units = "cm")
