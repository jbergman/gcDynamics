...
f3a <- ggplot(data, aes(x=mType, y=gamma, color=mType)) + geom_boxplot(outlier.size = 0.5,shape=21) + facet_wrap(.~pop, ncol = 8, nrow = 4) +
  ...
  geom_point(aes(x=mType,y=fullGamma), shape=18, size = 4, colour=c(rep("#377eb8",8*23), rep("#e41a1c",8*23), rep("#4daf4a",8*23)))

f3b <- ggplot(data, aes(x=cMmb, y=gamma, colour = mType)) + geom_point(size=1.25, shape=21, stroke=0, aes(fill=factor(mType))) +
